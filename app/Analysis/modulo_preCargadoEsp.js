...
export function especificAnalysis(userSymptoms,userOptions,modulo=modulo_datos){
    const umbral = 4;
    let interseccion = [];
    ...
    for (let i=0; i<userOptions.length; i++){ //Recorrido enfermedades
        let seleccionEnf = userOptions[i]-1;
        
        for(let j=0; j<modulo[seleccionEnf].length; j++){ //Recorrido de sintomas
            if(userSymptoms[j] < modulo[seleccionEnf][j]){
                valoresInt.push(userSymptoms[j]);
            }else{
                valoresInt.push(modulo[seleccionEnf][j]);
            }
            suma+=valoresInt[j];
        ...
        //Umbralización
        if(suma > umbral){
            datos=data.enfermedades[seleccionEnf+1];
            sumaSintomas.push(
                {
                    "enfermedad":datos.id, 
                    "valor":suma, 
